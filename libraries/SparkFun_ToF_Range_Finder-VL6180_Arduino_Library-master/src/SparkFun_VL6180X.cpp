...
void VL6180x::getIdentification(struct VL6180xIdentification *temp){
  ...
  temp->idModel =  VL6180x_getRegister(VL6180X_IDENTIFICATION_MODEL_ID, false);
  temp->idModelRevMajor = VL6180x_getRegister(VL6180X_IDENTIFICATION_MODEL_REV_MAJOR, false);
  temp->idModelRevMinor = VL6180x_getRegister(VL6180X_IDENTIFICATION_MODEL_REV_MINOR, false);
  temp->idModuleRevMajor = VL6180x_getRegister(VL6180X_IDENTIFICATION_MODULE_REV_MAJOR, false);
  temp->idModuleRevMinor = VL6180x_getRegister(VL6180X_IDENTIFICATION_MODULE_REV_MINOR, false);

  temp->idDate = VL6180x_getRegister16bit(VL6180X_IDENTIFICATION_DATE, false);
  temp->idTime = VL6180x_getRegister16bit(VL6180X_IDENTIFICATION_TIME, false);
}
...
uint8_t VL6180x::changeAddress(uint8_t old_address, uint8_t new_address){
  ...
  if( new_address > 127) return old_address;
   ...
   VL6180x_setRegister(VL6180X_I2C_SLAVE_DEVICE_ADDRESS, new_address, false);
   
   return VL6180x_getRegister(VL6180X_I2C_SLAVE_DEVICE_ADDRESS, false); 
}
...
uint8_t VL6180x::VL6180x_getRegister(uint16_t registerAddr, bool stop)
{
  uint8_t data;
...
uint16_t VL6180x::VL6180x_getRegister16bit(uint16_t registerAddr, bool stop)
{
  uint8_t data_low;
  uint8_t data_high;
  uint16_t data;
...
void VL6180x::VL6180x_setRegister(uint16_t registerAddr, uint8_t data, bool stop)
{
  Wire.beginTransmission( _i2caddress ); // Address set on class instantiation
  Wire.write((registerAddr >> 8) & 0xFF); //MSB of register address
  Wire.write(registerAddr & 0xFF); //LSB of register address
...
void VL6180x::VL6180x_setRegister16bit(uint16_t registerAddr, uint16_t data, bool stop)
{
  Wire.beginTransmission( _i2caddress ); // Address set on class instantiation
  Wire.write((registerAddr >> 8) & 0xFF); //MSB of register address
  Wire.write(registerAddr & 0xFF); //LSB of register address
