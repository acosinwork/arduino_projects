...
//
void mp3TF::upVolume () {
	static uint8_t upVolume_cmd [] = { 0X7E, 0xFF, 0x06, 0X04, 00, 00, 00, 0xFE, 0xED, 0XEF};
	sendCmd(upVolume_cmd);
}

//
void mp3TF::downVolume () {
	static uint8_t downVolume_cmd [] = { 0X7E, 0xFF, 0x06, 0X05, 00, 00, 00, 0xFE, 0xFA, 0XEF};
	sendCmd(downVolume_cmd);
}

//
void mp3TF::setVolume(uint16_t volume) {
	static uint8_t setVolume_cmd [10] = { 0X7E, 0xFF, 0x06, 0X06, 00, 00, 00, 0xFE, 0xee, 0XEF};
	setVolume_cmd[5] = (uint8_t)(volume >> 8);
	setVolume_cmd[6] = (uint8_t)(volume);
	sendCmd (setVolume_cmd);
}

//
void mp3TF::setVolume(int volume) {
	setVolume ((uint16_t)volume);
}
