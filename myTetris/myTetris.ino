...
byte buttonPress = 75; // check Capasitive Sensor

CapacitiveSensor   bt_up = CapacitiveSensor(A3, A0);       // 10 megohm resistor between pins 4 & 2, pin 2 is sensor pin, add wire, foil
CapacitiveSensor   bt_down = CapacitiveSensor(A3, A1);       // 10 megohm resistor between pins 4 & 6, pin 6 is sensor pin, add wire, foil
CapacitiveSensor   bt_left = CapacitiveSensor(A3, A2);       // 10 megohm resistor between pins 4 & 8, pin 8 is sensor pin, add wire, foil
CapacitiveSensor   bt_right = CapacitiveSensor(A3, A4);       // 10 megohm resistor between pins 4 & 2, pin 2 is sensor pin, add wire, foil
...
void setup() {
  ...
  Timer1.attachInterrupt( LEDRefresh ); // attach the service routine here
...
int readBut()
{
  ...
  if (bt_left.capacitiveSensor(30) > buttonPress)
  {
    //left
  ...
  if (bt_down.capacitiveSensor(30) > buttonPress)
  {
    //down
    bdelay = millis() + btdowndelay;
    return 4;
  }
  if (bt_right.capacitiveSensor(30) > buttonPress)
  {
    //right
    bdelay = millis() + btsidedelay;
    return 2;
  }
  if (bt_up.capacitiveSensor(30) > buttonPress)
  {
    //up
...
void LEDRefresh()
{
  ...
  */
  ...
  (currentCol) ? digitalWrite(rowPin + currentCol - 1, LOW) : digitalWrite(13, LOW);

  digitalWrite(latchPin, HIGH);

  digitalWrite(rowPin + currentCol, HIGH);

  (currentCol == 7) ? currentCol = 0 : ++currentCol;
...
digitalWrite(latchPin, LOW);
  ...
  digitalWrite(latchPin, HIGH);
/*
PORTD &= ~_BV(4); //pin 4 LOW  digitalWrite(latchPin, LOW);

unsigned int dataMask = 1;
for (byte i = 0; i < 16; ++i)
{

PORTD &= ~_BV(3); //pin 3 LOW  SCK


if (sendData & dataMask)
PORTD |= _BV(2); //pin 2 HIGH
else
PORTD &= ~_BV(2); //pin 2 LOW

dataMask <<= 1;

PORTD |= _BV(3); //pin 3 HIGH  SCK
}
PORTD |= _BV(4); //pin 4 HIGH  digitalWrite(latchPin, HIGH);
